-- Add grading columns to market_photos table
ALTER TABLE public.market_photos
ADD COLUMN grade integer CHECK (grade >= 1 AND grade <= 5),
ADD COLUMN grade_reason text,
ADD COLUMN graded_by uuid REFERENCES auth.users(id),
ADD COLUMN graded_at timestamp with time zone;

-- Add index for faster querying
CREATE INDEX idx_market_photos_grade ON public.market_photos(grade);
CREATE INDEX idx_market_photos_graded_at ON public.market_photos(graded_at);

-- Update weekly_rankings table to support average grade calculation
ALTER TABLE public.weekly_rankings
ADD COLUMN average_grade numeric(3,2),
ADD COLUMN total_grade_points integer DEFAULT 0;

-- Drop and recreate the update_weekly_rankings function with grade-based calculation
...
-- Add comment to clarify the new grading system
COMMENT ON COLUMN public.market_photos.grade IS 'Municipal officer grade (1-5 scale): 1=Poor, 5=Excellent';
COMMENT ON COLUMN public.weekly_rankings.average_grade IS 'Average grade from verified submissions (used for ranking)';